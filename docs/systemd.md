...
When **`shiphook.service` is already active**, running `shiphook` again will **restart** the systemd unit (so config changes are applied).

When **systemd starts `shiphook.service`**, the CLI won't try to restart it again (avoids restart loops). When you run `shiphook` manually from an interactive terminal, the CLI can restart the unit so the service comes up with updated config.
