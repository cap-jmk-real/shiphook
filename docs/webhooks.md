...
In multi-app mode (`apps:`), Shiphook first matches app route by **Host + path** and then validates that matched app's secret. Keys are isolated per app/repo.
If an app secret is omitted in config, Shiphook generates and persists one for that app on startup.
