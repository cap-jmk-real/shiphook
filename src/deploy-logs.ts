...
export async function writeDeployLogs(args: {
  ...
  const payload = {
    ...
    commit: args.result.commit ?? null,
    ref: args.result.ref ?? null,
    repository: args.result.repository ?? null,
  ...
  const textParts = [
    ...
    `commit: ${payload.commit ?? "n/a"}`,
    `ref: ${payload.ref ?? "n/a"}`,
    `repository: ${payload.repository ?? "n/a"}`,
    `ok: ${payload.ok}`,
